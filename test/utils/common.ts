...
export const deployUser = async function (initial_balance = 100, log=true): Promise<Account> {
    ...
    const {account: _user, tx} = await locklift.factory.accounts.addNewAccount({
        ...
        contract: "TestWallet",
        //Value which will send to the new acco unt from a giver
...
export const setupDeltexCluster = async function(owner: Address): Promise<Contract<UpexRootAbi>> {
    const signer = await locklift.keystore.getSigner('0');
    ...
    const {contract: _root, tx} = await locklift.tracing.trace(locklift.factory.deployContract({
        contract: 'DeltexCluster',
        initParams: {
            deploy_nonce: getRandomNonce(),
            tokenWalletCode: TokenWallet.code,
            tokenRootCode: TokenRoot.code,
        },
        constructorParams: {
            _owner: owner
        },
